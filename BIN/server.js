    ...
    if (FileFound) {
        ...
        fs.readFile('WebRoot/' + pathname.substr(1), function (err, data) {
            if (extension == "html" || extension == "htm" || extension == "js") {
                response.writeHead(200, { 'Content-Type': 'text/html', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' });
                response.end(data.toString());
            } else if (extension == "dat" || extension == "ts") {
                response.writeHead(200, { 'Content-Type': 'text/html', 'Content-Length': data.length, 'Accept-Ranges': 'bytes', 'Cache-Control': 'no-cache' });
                response.end(data.toString());
            } else if (extension == "css") {
                response.writeHead(200, { 'Content-Type': 'text/css', 'Content-Length': data.length, 'Accept-Ranges': 'bytes', 'Cache-Control': 'no-cache' });
                response.end(data.toString());
            } else if (extension == "png") {
                response.writeHead(200, { 'Content-Type': 'image/png', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' });
                response.end(data, 'binary');
            } else if (extension == "gif") {
                response.writeHead(200, { 'Content-Type': 'image/gif', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' });
                response.end(data, 'binary');
            } else if (extension == "bmp") {
                response.writeHead(200, { 'Content-Type': 'image/bmp', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' });
                response.end(data, 'binary');
            } else if (extension == "jpg") {
                response.writeHead(200, { 'Content-Type': 'image/jpg', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' });
                response.end(data, 'binary');
            }  else if (extension == "ico") {
                response.writeHead(200, { 'Content-Type': 'image/ico', 'Content-Length': data.length, 'Accept-Ranges': 'bytes' })
                response.end(data, 'binary');
            } else if (extension == "exe") {
                response.writeHead(200, { 'Content-Type': 'application/x-msdownload', 'Content-Length': data.length, 'Accept-Ranges': 'bytes', 'Cache-Control': 'no-cache' });
                response.end(data);
            }
        });
    } else {
...
io.on("connect", function (socket) {
        ...
        if (SQL_DB.tableExists(tableInfo.name)) {
            ...
            if (newtable != null) {
                ...
                if (newtable.data[tableInfo.timeRange] != null) {
                    var newtable = SQL_DB.GetTableClone(tableInfo.name)
                    socket.emit('updateTable', { name: newtable.name, data: newtable.data[tableInfo.timeRange].data, error: newtable.data[tableInfo.timeRange].error, ts: newtable.data[tableInfo.timeRange].ts });
                } else if (newtable.data["None"] != null) {
                    socket.emit('updateTable', { name: newtable.name, data: newtable.data["None"].data, error: newtable.data["None"].error, ts: newtable.data["None"].ts });
                } else {
    ...
    socket.on("getTVs", function () {
        var clients = getSockets();
        var TVlist = [{ k: "Un-Paired", v: "unpair" }]
        for (i = 0; i < clients.length; i++) {
            if (clients[i].isTV && clients[i].pairedDeviceKey == "") {
                TVlist.push({ k: clients[i].tvName, v: clients[i].pairingKey })
            }
        }
    ...
    socket.on("tvPaired", function (params) {
        ...
        for (i = 0; i < clients.length; i++) {
            if (clients[i].pairingKey == socket.pairedDeviceKey) {
                clients[i].emit('cloneToTV', { cmd: 'changeGroup', params: params.group })
                clients[i].emit('cloneToTV', { cmd: 'changePage', params: params.page })
                clients[i].emit('cloneToTV', { cmd: 'changeGlobalTimerange', params: params.timeRange })
                break;
            }
        }
    })
...
SimplePerfMon.Start(31, function () {
    ...
    monTick++
    if (monTick > 0) {
        io.emit('tableUpdated', { name: "Server_Perf", timeRange: "None" });
        io.emit('tableUpdated', { name: "cpu", timeRange: "None" });
        io.emit('tableUpdated', { name: "ram", timeRange: "None"});
        monTick = 0
    }
})