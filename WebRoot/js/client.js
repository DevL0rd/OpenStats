...
function updateItem(Item) {
    ...
    socket.emit('updateTable', { name: Item.dataSet, timeRange: Item.timeRange});
}
...
$('#reportrange').on('apply.daterangepicker', function (ev, picker) {
    ...
    if (tvKey != "") {
        socket.emit('cloneToTV', { token: tvKey, cmd: "changeGlobalTimerange", params: GlobalTimerange })
...
function showDelete() {
    $(".trash").effect('slide', { direction: 'right', mode: 'show' }, 500);
}
...
function loadItem(item, itemIndex) {
    ...
    try {
        ...
        if (item.type == "pie") {
            ...
            item.options.animation = { duration: 600, easing: 'inAndOut', "startup": true }
            //Perfectly size data
            item.options.width = $("#Item_" + itemIndex).width
            item.options.height = $("#Item_" + itemIndex).height
            //Apply chart colors
...
function filterPage(filterStr) {
    ...
    if (filterStr != "" && filterStr != lastFilterStr && filterStr.length > 1 && isNaN(filterStr)) {
        if (tvKey != "") {
            socket.emit('cloneToTV', { token: tvKey, cmd: "search", params: filterStr })
        }
        SizeByIndex = true
        ...
        if (lastFilterStr != filterStr) {
            ...
            $(".btnsettings").show()
            if (tvKey != "") {
                socket.emit('cloneToTV', { token: tvKey, cmd: "clssearch", params: "" })
            }
        }
...
function changePage(nP) {
    ...
    if (tvKey != "") {
        socket.emit('cloneToTV', { token: tvKey, cmd: "changePage", params: nP })
        clearTimeout(pairedTvTimeoutID)
        pairedTvTimeoutID = setTimeout(function () {
            unPair()
        }, tvDisconnectTimout)
...
function changeGroup(nG) {
    ...
    if (tvKey != "") {
        socket.emit('cloneToTV', { token: tvKey, cmd: "changeGroup", params: nG })
        clearTimeout(pairedTvTimeoutID)
        pairedTvTimeoutID = setTimeout(function () {
            unPair()
        }, tvDisconnectTimout)
...
socket.on('pairTV', function () {
    ...
    filterPage("")
    socket.emit('tvPaired', { group: Group, page: Page, timeRange: GlobalTimerange})
